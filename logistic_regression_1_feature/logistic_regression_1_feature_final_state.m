...
for i = 1 : size(mesh_theta_1)(1)
	for j = 1 : size(mesh_theta_1)(2)
		...
		mesh_J(i, j) = cost_function_log(X, mesh_theta, y);
	end
end
...
% Save history of theta
theta_1_history(1) = theta(1);
theta_2_history(1) = theta(2);

for i = 1 : NB_ITERATION
	% Compute new theta and J
	[theta, J] = gradient_descent_log (X, theta, y, ALPHA);
	J_history(i + 1) = J;
	theta_1_history(i + 1) = theta(1);
	theta_2_history(i + 1) = theta(2);
end

% Plot J
subplot(2, 2, 1);
plot(1:length(J_history), J_history, 'o');
xlabel('Iteration number');
ylabel('Cost')
grid on;
...
subplot(2, 2, 4);
plot(theta_1_history, theta_2_history, 'o');
